...
class AgentMiller(AbstractAgent):
    ...
    def __init__(self, agent_id, probability, forgive_steps: int = 2):
        self.forgive_steps = forgive_steps
    ...
    def get_action_ipd_noisy(self, total_rounds: int, history: list[tuple[Action, Action]]) -> Action:
        ...
        x = 1 if total_rounds == 10 else 
        if len(history) >= 2 and history[-2][1] == Action.COOPERATE:
            return Action.COOPERATE
